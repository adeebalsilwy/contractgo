...
class ContractsController extends Controller
{
        ...
        $contracts = $contracts->orderBy($sort, $order)
            ...
            ->through(function ($contract) use ($canEdit, $canDelete, $canCreate) {
                ...
                if ($contract->workflow_status === 'approved' && !$contract->is_archived) {
                    $actions .= '<a href="javascript:void(0);" class="archive-contract" data-id="' . $contract->id . '" title="Archive Contract"><i class="bx bx-archive text-info mx-2"></i></a>';
                } elseif ($contract->is_archived) {
                    $actions .= '<a href="javascript:void(0);" class="unarchive-contract" data-id="' . $contract->id . '" title="Unarchive Contract"><i class="bx bx-unarchive text-warning mx-2"></i></a>';
                }
    ...
    public function archive($id)
    {
        try {
            ...
            // Check if user is authorized to archive this contract
            if (!isAdminOrHasAllDataAccess() && $this->user->id != $contract->final_approver_id) {
                abort(403, 'Unauthorized to archive this contract');
            }

            // Check if the contract can be archived (must be fully approved)
            if ($contract->workflow_status !== 'approved') {
                return response()->json(['error' => true, 'message' => 'Contract cannot be archived as it is not fully approved.']);
            }
            ...
            $contract->update([
                ...
                'workflow_notes' => ($contract->workflow_notes ?? '') . "\nArchived on " . now() . " by " . $this->user->first_name . " " . $this->user->last_name
            ]);

            Session::flash('message', 'Contract archived successfully.');
            return response()->json(['error' => false, 'message' => 'Contract archived successfully.']);
        } catch (\Exception $e) {
    ...
    public function unarchive($id)
    {
        try {
            ...
            // Check if user is authorized to unarchive this contract
            if (!isAdminOrHasAllDataAccess()) {
                abort(403, 'Unauthorized to unarchive this contract');
            }
            ...
            $contract->update([
                ...
                'archived_at' => null,
                'archived_by' => null,
                'workflow_status' => 'approved', // Return to approved status
                'workflow_notes' => ($contract->workflow_notes ?? '') . "\nUnarchived on " . now() . " by " . $this->user->first_name . " " . $this->user->last_name
            ]);

            Session::flash('message', 'Contract unarchived successfully.');
            return response()->json(['error' => false, 'message' => 'Contract unarchived successfully.']);
        } catch (\Exception $e) {
            return response()->json(['error' => true, 'message' => $e->getMessage()]);
        }
    }
    ...
    /**
     * List archived contracts
     */
    public function archived()
    {
        $contracts = isAdminOrHasAllDataAccess() 
            ? $this->workspace->contracts()->where('is_archived', true)
            : $this->user->contracts()->where('is_archived', true);
