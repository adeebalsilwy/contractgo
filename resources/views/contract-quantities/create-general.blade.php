...
@section('content')
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5><?= get_label('create_contract_quantity', 'Create Contract Quantity') ?></h5>
                    </div>
                    <div class="card-body">
                        <form id="contract_quantity_form" action="{{ route('contract-quantities.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label"><?= get_label('contract', 'Contract') ?> <span class="text-danger">*</span></label>
                                    <select class="form-select select2" id="contract_id" name="contract_id" required onchange="loadContractClient(this.value)">
                                        <option value=""><?= get_label('select_contract', 'Select Contract') ?></option>
                                        @foreach($contracts as $contract)
                                            <option value="{{ $contract->id }}" data-client-id="{{ $contract->client->id ?? '' }}">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label class="form-label"><?= get_label('client', 'Client') ?> <span class="text-danger">*</span></label>
                                    <select class="form-select select2" id="client_id" name="client_id" required disabled>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label class="form-label"><?= get_label('item', 'Item') ?> <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="item_description" name="item_description" required value="">
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label class="form-label"><?= get_label('requested_quantity', 'Requested Quantity') ?> <span class="text-danger">*</span></label>
                                    <input type="number" class="form-control" id="requested_quantity" name="requested_quantity" min="0" step="0.01" required value="1">
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="unit_price" name="unit_price" min="0" step="0.01" value="0">
                                </div>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <input type="file" class="form-control" id="supporting_documents" name="supporting_documents[]" multiple accept=".pdf,.jpg,.jpeg,.png">
                                    <small class="text-muted"><?= get_label('supporting_documents_help', 'Upload supporting documents (PDF, JPG, PNG) - Max 10MB each') ?></small>
                                </div>
                            </div>
                            
                            <div class="d-flex justify-content-end">
                                <button type="submit" class="btn btn-primary me-2"><?= get_label('submit', 'Submit') ?></button>
                                <button type="reset" class="btn btn-secondary"><?= get_label('reset', 'Reset') ?></button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            if($('.select2').length) {
                $('.select2').select2({
                    placeholder: '<?= get_label('select_contract', 'Select Contract') ?>',
                    allowClear: true
                });
            }
            ...
            $('#contract_quantity_form').on('submit', function(e) {
                e.preventDefault();
                ...
                // Add contract_id to formData if not already present
