...
@section('page_title', get_label('create_item_pricing', 'Create Item Pricing'))
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">{{ get_label('create_item_pricing', 'Create Item Pricing') }}</h4>
                        <a href="{{ route('item-pricing.index') }}" class="btn btn-secondary">
                            <i class="fas fa-arrow-left"></i> {{ get_label('back', 'Back') }}
                        </a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('item-pricing.store') }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="number" step="0.01" name="price" id="price" class="form-control" placeholder="{{ get_label('enter_price', 'Enter Price') }}" value="{{ old('price') }}" required>
                                        @error('price')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group mb-3">
                                        ...
                                        <textarea name="description" id="description" class="form-control" rows="3" placeholder="{{ get_label('enter_description', 'Enter Description') }}">{{ old('description') }}</textarea>
                                        @error('description')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
