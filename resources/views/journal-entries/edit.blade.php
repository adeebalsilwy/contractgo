@extends('layout')
@section('title')
    <?= get_label('edit_journal_entry', 'Edit Journal Entry') ?>
@endsection
@section('content')
    @php
    $menu = 'finance';
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5><?= get_label('update_journal_entry', 'Update Journal Entry') ?></h5>
                    </div>
                    <div class="card-body">
                        <form id="journalEntryForm">
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select name="entry_type" class="form-control" required>
                                        <option value="journal" {{ $journalEntry->entry_type == 'journal' ? 'selected' : '' }}><?= get_label('journal', 'Journal') ?></option>
                                        <option value="invoice" {{ $journalEntry->entry_type == 'invoice' ? 'selected' : '' }}><?= get_label('invoice', 'Invoice') ?></option>
                                        <option value="payment" {{ $journalEntry->entry_type == 'payment' ? 'selected' : '' }}><?= get_label('payment', 'Payment') ?></option>
                                        <option value="receipt" {{ $journalEntry->entry_type == 'receipt' ? 'selected' : '' }}><?= get_label('receipt', 'Receipt') ?></option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select name="status" class="form-control" required>
                                        <option value="pending" {{ $journalEntry->status == 'pending' ? 'selected' : '' }}><?= get_label('pending', 'Pending') ?></option>
                                        <option value="posted" {{ $journalEntry->status == 'posted' ? 'selected' : '' }}><?= get_label('posted', 'Posted') ?></option>
                                        <option value="reversed" {{ $journalEntry->status == 'reversed' ? 'selected' : '' }}><?= get_label('reversed', 'Reversed') ?></option>
                                        <option value="cancelled" {{ $journalEntry->status == 'cancelled' ? 'selected' : '' }}><?= get_label('cancelled', 'Cancelled') ?></option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row mb-3">
                                <div class="col-sm-10 offset-sm-2">
                                    <button type="submit" class="btn btn-primary"><?= get_label('update_journal_entry', 'Update Journal Entry') ?></button>
